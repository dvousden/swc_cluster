...
resultspaths = fullfile(savedir_path, '/processed_2passes.mat');
...
`pwd`/slurm_logs/pipeline_register-${SLURM_JOB_ID}

% pc.JobStorageLocation = strcat(pwd,'/slurm', getenv('SLURM_JOB_ID'))
pc.JobStorageLocation = strcat(pwd,'/slurm_logs/pipeline_register-', getenv('SLURM_JOB_ID'))
